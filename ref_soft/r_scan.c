...
/*
=============
D_DrawSkyOverlaySpans

Truecolor sky variant of D_DrawSpans16. Uses the identical perspective-correct
texcoord stepping, but instead of writing an 8-bit texel it stamps the sky
sentinel index into the paletted view buffer and the matching RGB565 sample
(from sw_sky565_cur, a 256x256 truecolor face) into the parallel overlay. The
libretro output stage composites the overlay over the paletted frame. A 565
value of 0 is nudged to 1 so it can never collide with the overlay's cleared
state. Sky is a small fraction of the screen, so this is intentionally not
unrolled.
=============
*/
void D_DrawSkyOverlaySpans (espan_t *pspan)
{
	unsigned short  *pbase565 = sw_sky565_cur;
	byte            *pdest;
	unsigned short  *povl;
