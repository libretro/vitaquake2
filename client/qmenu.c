...
void Field_Draw( menufield_s *f )
{
	...
	Draw_Char(f->generic.x + f->generic.parent->x + 16 * scale,
			(f->generic.y + f->generic.parent->y - 4) * scale, 18, scale);
	Draw_Char(f->generic.x + f->generic.parent->x + 16 * scale,
			(f->generic.y + f->generic.parent->y + 4) * scale, 24, scale);

	Draw_Char((f->generic.x + f->generic.parent->x + 24 * scale) + (f->visible_length * 8 * scale),
	...
	{
		Draw_Char((f->generic.x + f->generic.parent->x + 24 * scale) + (i * 8 * scale),
...
void Menu_DrawString( int x, int y, const char *string )
{
	...
	{
		Draw_Char( ( x + i*8*scale ), scale*y, string[i], scale );
	}
}
...
void Menu_DrawStringDark( int x, int y, const char *string )
{
	...
	for ( i = 0; i < strlen( string ); i++ )
	{
		Draw_Char( ( x + i*8*scale ), y*scale, string[i] + 128, scale );
	}
}
...
void Menu_DrawStringR2L( int x, int y, const char *string )
{
	...
	for ( i = 0; i < strlen( string ); i++ )
	{
		Draw_Char( ( x - i*8*scale ), scale*y, string[strlen(string)-i-1], scale );
	}
}
...
void Menu_DrawStringR2LDark( int x, int y, const char *string )
{
	...
	for ( i = 0; i < strlen( string ); i++ )
	{
		Draw_Char( ( x - i*8*scale ), y*scale, string[strlen(string)-i-1]+128, scale );
	}
}
...
void Slider_Draw( menuslider_s *s )
{
	...
	if ( s->range > 1)
		s->range = 1;
	Draw_Char( s->generic.x + s->generic.parent->x + RCOLUMN_OFFSET * scale, scale * (s->generic.y + s->generic.parent->y), 128, scale);
	for ( i = 0; i < SLIDER_RANGE; i++ )
		Draw_Char( scale * RCOLUMN_OFFSET + s->generic.x + i*8 + s->generic.parent->x + 8, scale*(s->generic.y + s->generic.parent->y), 129, scale);
	Draw_Char( scale * RCOLUMN_OFFSET + s->generic.x + i*8 + s->generic.parent->x + 8, scale*(s->generic.y + s->generic.parent->y), 130, scale);
	Draw_Char( ( int ) ( 8 + scale * RCOLUMN_OFFSET + s->generic.parent->x + s->generic.x + (SLIDER_RANGE*scale-1)*8 * s->range ), (s->generic.y + s->generic.parent->y)*scale, 131, scale);
}
